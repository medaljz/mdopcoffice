...
for(const name of ['INSTALL.md','share-guide.md','AGENT-INSTALL.md','RELEASE-STATUS.md','ASSETS.md','office-preview.png'])await copy('docs/'+name);
...
await writeFile(path.join(dest,'README.md'),(await readFile(path.join(dest,'docs/share-guide.md'),'utf8')).replaceAll('(INSTALL.md)','(docs/INSTALL.md)').replaceAll('(ASSETS.md)','(docs/ASSETS.md)'));await writeFile(path.join(dest,'AGENTS.md'),'# OPC 安装与修改\n\n先阅读 docs/AGENT-INSTALL.md 和 docs/RELEASE-STATUS.md。保留用户的源码、公司与人物设置；绑定当前电脑自己的 Agent。未经真实验收不宣称派发或自动启动成功。不得复制其他安装的账号或聊天记录。\n');
